...
s32 * StripToHostName(memory_arena *memoryArena, s32* host)
{
	...
	do
	{
		...
		switch (sToken.type)
		{
			case SEARCH_COLON:
			{
				char* http = PushMidString(memoryArena,(char*)host, 0, sTokenizer.count);
	...
	if (found)
	{
		result = PushMidString(memoryArena,host, slashPos+1, sTokenizer.count - 1);
	}
	else
	if (httpFound && found == false)
	{
		result = PushMidString(memoryArena,host, slashPos+1, strlen(host));
	}
	else
	{
		if (slashPos != -1)
		{
			result =PushMidString(memoryArena,host, 0, slashPos - 1);
		}
		else {
			result = PushMidString(memoryArena,host, 0, strlen(host));
		}
	}
